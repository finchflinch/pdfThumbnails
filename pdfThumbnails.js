...
// needed a way to generate thumbnails for particular elements generated at run time (not present during when DOM was loaded)
// i call this function during the onload event for the required element.
var genSingleThumbnail = function(singleElement) {
    var worker = null;
    var loaded = false;
    var renderQueue = [];

    // select all img elements with data-pdf-thumbnail-file attribute
    var nodesArray = [singleElement];
            ...
            pdfjsLib.getDocument({url: filePath, worker: worker}).promise.then(function (pdf) {
                pdf.getPage(1).then(function (page) {
                    ...
                    page.render({
                        ...
                        element.src = canvas.toDataURL();
                        // to prevent a recursive/infinite calling set the on load or any other event through which you are calling this function, set it to blank
                        element.onload = "";
                    });
                }).catch(function() {
                    console.log("pdfThumbnails error: could not open page 1 of document " + filePath + ". Not a pdf ?");
